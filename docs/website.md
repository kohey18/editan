...
The requested GPT image 2.5 artwork is pending model availability/approval. The built-in image tool in this session exposes no model selector. No generated image is claimed to have used that model, and no substitute artwork is silently shipped. The current site uses the project's actual app imagery. Once an approved social card is generated and reviewed, save it to `docs/assets/social.png`, record the prompt and generation method, and add absolute Open Graph/X image metadata to both language pages.
